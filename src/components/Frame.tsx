 ...
 * than a border so the lines never scale with the viewport.
 */
export function Frame({ hidden }: FrameProps) {
  const line = `fixed z-20 bg-line transition-opacity duration-[400ms] ${
    hidden ? "opacity-0" : "opacity-100"
  }`;
  const horizontal = "left-frame-x h-[0.5px] w-[calc(100%-2*var(--frame-x))] sm:h-px";
  const vertical = "top-frame-y h-[calc(100%-2*var(--frame-y))] w-[0.5px] sm:w-px";
