...
/** Matches the collapse animation in `src/lib/blackhole.ts`. */
    ...
    <main className="h-full overflow-hidden bg-bg text-fg">
      ...
      {phase === "collapsed" && (
        <button
          ...
          className="fixed right-content-x top-content-y z-20 animate-fadeIn border border-line px-2 py-0.5 text-[0.65rem] font-normal tracking-wide text-fg sm:left-1/2 sm:right-auto sm:-translate-x-1/2 sm:px-3 sm:py-1 sm:text-xs"
        >
          Home
        </button>
      )}
      ...
      {active === "home" && (
        <BlackHole exploding={collapsed} onExplode={() => setPhase("collapsing")} />
      )}
    </main>
  );
}
